# Graph Report - rag-techniques  (2026-07-22)

## Corpus Check
- 67 files · ~394,757 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1321 nodes · 1231 edges · 185 communities (46 shown, 139 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 27 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c2f21691`
...
2. `System architecture` - 10 edges
3. `6. Phase-by-Phase Architecture` - 9 edges
4. `render_all()` - 8 edges
5. `CLAUDE.md — Agent Instructions` - 8 edges
6. `Human Intervention Ledger (read this before starting)` - 8 edges
7. `Human Intervention Ledger (read this before starting)` - 8 edges
8. `4. Implementation Guardrails (Binding)` - 7 edges
9. `Benchmark design` - 7 edges
10. `Working in this repo` - 7 edges

## Surprising Connections (you probably didn't know these)
- `ingest_one()` --calls--> `fetch_filing()`  [INFERRED]
  project/ingest/run_ingestion.py → project/ingest/fetch_filings.py
- `ingest_one()` --calls--> `parse_filing()`  [INFERRED]
  project/ingest/run_ingestion.py → project/ingest/parse_filing.py
- `test_parse_filing_calls_llamaparse_and_caches()` --indirect_call--> `parse_filing()`  [INFERRED]
  project/tests/test_parse_filing.py → project/ingest/parse_filing.py
- `test_parse_filing_reparses_if_cache_older_than_48h()` --indirect_call--> `parse_filing()`  [INFERRED]
  project/tests/test_parse_filing.py → project/ingest/parse_filing.py
- `test_parse_filing_skips_reparse_within_48h()` --indirect_call--> `parse_filing()`  [INFERRED]
  project/tests/test_parse_filing.py → project/ingest/parse_filing.py
...
## Communities (185 total, 139 thin omitted)
...
Cohesion: 0.08
Nodes (4): _dumps(), SQLite access layer: five isolated tables, WAL mode, JSON-in-TEXT convention.  S, upsert_result(), Randomized 20-section parsing validation audit.  Samples nodes across the corpus
...
Cohesion: 0.22
Nodes (4): BaseException, _is_rate_limit_error(), Resilient async Groq wrapper: tenacity backoff on 429 + bounded concurrency.  Se, _FakeResponse
...
## Knowledge Gaps
- **970 isolated node(s):** `rag-techniques-benchmark`, `_FakeResponse`, `Repository Layout`, ``resources/` Subdirectory Reference`, `Reading Order for `resources/specs/`` (+965 more)
...
- **Why does `CLAUDE.md — Agent Instructions` connect `Guardrails and Model Routing` to `4. Implementation Guardrails (Binding)`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Why does `Working in this repo` connect `Working in this repo` to `quickstart.md`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
...
- **Should `Phase Plan.md` be split into smaller, more focused modules?**
  _Cohesion score 0.05714285714285714 - nodes in this community are weakly interconnected._