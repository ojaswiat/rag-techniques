...
(async () => {
    ...
    async function loginUser(launchSettings, getRolesFunc) {
        ...
        const [token, { roles, coursesComplete }] = await Promise.all([getToken(), getRolesFunc(launchSettings)]);

        return loginUserCommon(launchSettings, roles, {
            token: token.token,
            courses_complete: coursesComplete,
        });
    }
    ...
    async function setNonAuthorizedUserVariables(launchSettings, host, roles) {
        ...
        window.var_show_tab_initial = launchSettings.supportNonAuthorizedUsers;
        window.var_show_tab = launchSettings.supportNonAuthorizedUsers;
    ...
    async function fetchAllPages(url) {
        const MAX_COURSE_PAGES = 25; // safety cap; a >2500-course user is rare and handled by the Phase 3 sync
        let items = [];
        let next = url;
        let pages = 0;
        while (next) {
            if (pages >= MAX_COURSE_PAGES) {
                return { items, complete: false }; // hit cap before exhausting -> not complete
            }
            const res = await fetch(next);
            if (!res.ok) {
                return { items, complete: false }; // transient/partial -> not complete
            }
            ...
            const match = link && link.match(/<([^>]+)>\s*;\s*rel="next"/);
    ...
    async function getRoles(launchSettings) {
        // Paginate the full course set with explicit enrollment state so we can assert completeness.
        // NOTE: enrollment_state[]=completed is intentionally omitted for cost/scale reasons; concluded
        // enrollments are handled by Phase 3 and could later be made opt-in via a setting.
        const url =
            '/api/v1/users/self/courses?include[]=account&include[]=term' +
            '&enrollment_state[]=active&enrollment_state[]=invited&per_page=100';
        const { items: coursesWithAccounts, complete: fetchComplete } = await fetchAllPages(url);
        let coursesWithFilteredAccounts = [];
        let coursesComplete = fetchComplete;

        if (launchSettings.restrictedToAccount) {
            // Dedup: build a unique Set of account ids and fire ONE visibility call per distinct account,
            // instead of one call per course. With pagination this prevents thousands of redundant calls.
            const uniqueAccountIds = [
                ...new Set(
                    coursesWithAccounts
                        .filter((course) => course.account && course.account.id != null)
                        .map((course) => course.account.id)
                ),
            ];

            let anyVisibilityCallFailed = false;

            const requests = uniqueAccountIds.map(async (accountId) => {
                const params = {
                    id: accountId,
                ...
                try {
                    const res = await fetch(getEesyServerURL('/rest/public/canvas/account/visibility', params));
                    if (!res.ok) {
                        anyVisibilityCallFailed = true;
                        return null;
                    }
                    return res.json();
                } catch (e) {
            ...
            accountsVisibility.forEach((account) => {
                if (account && account.visible) allowedAccounts[account.id] = true;
            });
            ...
            // Only assert completeness when we both retrieved the full course set AND successfully
            // evaluated visibility for every distinct account; otherwise stay additive-only server-side.
            coursesComplete = fetchComplete && !anyVisibilityCallFailed;
        } else {
        ...
        return { roles: Object.values(roles), coursesComplete };
    }
    ...
    function loadScript(url) {
        return new Promise((resolve, reject) => {
            const safeUrl = new URL(url, window.location.origin); // Throws TypeError if it can't parse url

            if (safeUrl.protocol !== 'https:') {
                reject(new Error(`Refusing to load non-https script: ${url}`));
                return;
            }

            const scriptTag = document.createElement('script');
            scriptTag.src = encodeURI(safeUrl.toString());
