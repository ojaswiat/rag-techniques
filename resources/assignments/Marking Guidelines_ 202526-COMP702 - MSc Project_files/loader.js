...
document.addEventListener('click', function (e) {
	const link = e.target.closest('a[data-helpitem-guid]');
	if (!link) return;
	e.preventDefault();
	const guid = decodeURIComponent(link.getAttribute('data-helpitem-guid') || '');
	if (link.getAttribute('data-helpitem-type') === 'articles') {
		handleHelpItemArticle(guid);
	} else {
		handleHelpItemByGuid(guid);
	}
});
