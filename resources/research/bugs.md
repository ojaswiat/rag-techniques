...
### 1. Dataset-generation orchestrator only ever saw 9 of the 13 in-scope filings (2026-08-11)
...
### 2. Real Groq API failures crashed the whole dataset-generation run instead of being retried (2026-08-11)
...
### 3. LLMFactory's retry/backoff wrapper was silently never applied (2026-08-11)
...
### 4. Chroma silently dropped real filing data on duplicate node IDs (2026-08-10)
...
### 5. ChromaVectorStore's reserved metadata key silently overwrote our own `document_id` (2026-08-10)
...
### 6. Structural metadata leaked into P1's embedded/reranked text (2026-08-10)
...
### 7. `storage_root` late-binding broke test isolation and silently reused production paths (2026-08-10)
...
3. **Impact / Severity:** Medium -- a test-isolation bug, not a production-data bug on its own, but it's exactly the mechanism that let bug #2 (Chroma poisoning) happen in the first place.
...
### 8. GQ hand-labelling accepted invalid scores and silently no-op'd on typos (2026-07-29)
...
### 9. P3 build logged zero token cost despite generating real summaries (2026-08-04)
...
### 10. LLM temperature silently fell back to 0.1, not the mandated 0 (2026-08-04)
...
### 11. Empty filing-list argument was indistinguishable from "not provided" (2026-08-04)
...
### 12. No duplicate-question check let near-identical questions into the benchmark set (2026-08-04)
