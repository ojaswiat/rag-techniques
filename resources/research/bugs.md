...
### 14. Three hardcoded diagnostic figures in the results report were unreproducible, and one supported a wrong conclusion (2026-09-12)

1. **Bug:** The "Why the structural pipeline fails" paragraph in `build_report.py` asserted three numbers as literal text rather than reading them from the database: retrieval diversity of `0.867`, a median retrieved node at `ordinal 818`, and a median gold node at `ordinal 406`. No script in the repository computed any of them, so none could be checked or regenerated. Recomputing from `benchmark.db` gave diversity `0.506` and a median miss distance of `324` nodes. The `818` figure was worse than merely wrong: it is an absolute node index pooled across filings that run from 682 to 3,647 nodes, so it exceeds the entire length of five of the thirteen filings and cannot be compared to a pooled gold index at all. Worst of all, the sentence those two ordinals supported, "the right region of the filing, the wrong node within it", is false. P3's median miss is 29.2% of the filing it was searching, against 3.0% for BM25, and only 6.9% of the nodes it returns are evidence nodes against 29.1% for BM25. It is not landing in the right region.
2. **Root cause:** The figures came from an exploratory analysis run in a working session and were typed into the report prose by hand. Every other number in that report is read from `results` at render time, so this paragraph was the single place where the report asserted something it could not reproduce, and nothing in the build would ever have flagged it. The pooling error followed from using a raw node index as if the corpus were one document.
3. **Impact / Severity:** High. The published results report drew its central explanation of the P3 result from a statistic that overstated diversity by 71%, used an incomparable position measure, and stated a mechanism the data contradicts. A reader asking how 818 was obtained would have found no answer in the repository. The headline P1/P2/P3 ranking is untouched, since none of these figures feed any metric, table or figure.
4. **Fix:** Added `project/retrieval_diagnostics.py`, which computes diversity as distinct node ids over returned slots across the benchmark, and miss distance as the number of nodes to the nearest ground-truth node, reported both absolutely and as a share of the filing it was measured in so filings of different lengths stay comparable. Node position is read from the node id, since `ingest/node_builder.build_nodes` stamps an incrementing counter as it walks the parsed filing top to bottom and `nodes.source_page_num` was never populated. `build_report.py` now computes the paragraph's figures through it, and the mechanism sentence was rewritten to say what the numbers actually show: a retriever biased late, with a median returned node at 0.596 of the way through a filing whose evidence sits at 0.354. 19 tests added in `tests/test_retrieval_diagnostics.py`, covering both properties the diagnosis rests on: that diversity is counted corpus wide rather than within a row, where it is always 1.000 and says nothing, and that miss distance is normalised per filing.
5. **Found via:** A consistency check across the two results deliverables. Generating `findings.html` required recomputing the same diagnostics, which disagreed with the report; grepping for the figures showed they were hardcoded literals with no computing script anywhere in the repository.

---

### 15. The per-filing lead sentence in the findings page carried a typed tie count that was wrong (2026-09-12)

1. **Bug:** The "Filing by filing" section of `build_findings_page.py` introduced its heatmap with the sentence "BM25 leads outright on six, the vector pipeline on three, and they tie on two more." Those three counts were typed into the template as words rather than computed. The first two are correct; the tie count is not. Counting from `by_document` gives 6 filings where BM25 leads, 3 where the vector pipeline leads, and **4 ties**, not 2. The three counts as published summed to 11 rather than the 13 filings in the corpus, so the sentence silently dropped two filings.
2. **Root cause:** The same failure mode as entry 14, in the other deliverable. The counts were read off a working query during drafting and written into the prose as literals. Every other number in the page is substituted from the aggregation at render time, which is why nothing caught it: a typed word is not a placeholder, so the template's own placeholder guard had nothing to flag, and no test asserted the sentence against the data behind the chart it introduces.
3. **Impact / Severity:** Low for the conclusions, moderate for the deliverable. No metric, table, figure or statistical test is affected, and the two counts that carry the actual claim, that BM25 leads twice as many filings as the vector pipeline, were correct. What was wrong was an arithmetic statement a reader could check in ten seconds against the heatmap printed directly beneath it, in a page written to be the sole source for a dissertation write-up.
4. **Fix:** Added `filing_leads()` to `build_findings_page.py`, which counts leads, ties, last places and three-way ties from the same `by_document` aggregation the heatmap draws from, so the sentence and the chart cannot disagree. The prose now substitutes those counts. Two tests were added: one asserts the published tie count against a count taken independently from the fixture database, and one flattens every filing to a tie and requires the sentence to follow. Both fail when the sentence is reverted to the typed value, confirming they are not vacuous.
5. **Found via:** Verifying every prose claim in the expanded findings page against the database before publishing it, rather than only the substituted fields. The check that surfaced it was arithmetic: the three counts in the sentence should sum to the corpus size, and they summed to 11 of 13.
